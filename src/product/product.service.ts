...
export class ProductService {
	...
	async addNewProductToTheDatabase(product: any){
		try {
			...
			return { phoneNumber: prod.phoneNumber, price: prod.price, email: prod.email, name: prod.name, color :prod.color, address: prod.address}
		} catch (error) {
			if (error.code === 11000) {
	...
	async getOneproductFromTheDatabase(productId: any){
		const prod = await this.__productModel.findOne({_id:productId})
		return prod;
	}
